// Helper: generate a formatted date string relative to now
const ago = (days, hour = 9, minute = 0) => {
  const d = new Date()
  d.setDate(d.getDate() - days)
  d.setHours(hour, minute, 0, 0)
  return d.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) +
    ' ' + String(hour).padStart(2, '0') + ':' + String(minute).padStart(2, '0')
}

const future = (days, hour = 12, minute = 0) => {
  const d = new Date()
  d.setDate(d.getDate() + days)
  d.setHours(hour, minute, 0, 0)
  return d.toISOString().replace('T', ' ').slice(0, 16) + ' UTC'
}

export const ships = {
  invictus: {
    id: 'invictus',
    name: 'Invictus',
    flag: '\u{1F1F2}\u{1F1ED}',
    imo: '9819870',
    mmsi: '311000686',
    shipId: 'f8f6a2c9-9c04-777e-ecc4-6bcf114315ac',
    latestEvent: 'AIS',
    shipType: 'Tanker',
    aisInfo: {
      latitude: '24.119738',
      longitude: '32.149799',
      width: '227',
      length: '314',
      shipType: 'Tanker',
      buildYear: 'No info',
      heading: '227',
      draft: '5.9m',
      avgSpeed: '6.9',
      maxSpeed: '8.4',
      latestSpeed: `8 (${future(-1, 9, 3)})`,
      destination: 'No info',
      eta: 'No info',
    },
    synMaxInfo: {
      objectId: '391028475',
      imageCapturedTime: '2024-11-12 06:18:33 UTC',
      imageSource: 'Planet Scope',
      status: 'Confirmed',
      latitude: '24.119738',
      longitude: '32.149799',
      heading: '227',
      shipLength: '314',
      shipWidth: '227',
      shipType: 'Tanker',
      shipSubtype: 'VLCC',
    },
  },
  tiffani: {
    id: 'tiffani',
    name: 'Tiffani',
    flag: '\u{1F1F1}\u{1F1F7}',
    imo: '9456123',
    mmsi: '256789012',
    shipId: 'a3b2c1d0-e5f4-6789-abcd-0123456789ef',
    latestEvent: 'Light',
    shipType: 'Bulk Carrier',
    aisInfo: {
      latitude: '17.432901',
      longitude: '63.881245',
      width: '32',
      length: '190',
      shipType: 'Bulk Carrier',
      buildYear: '2015',
      heading: '142',
      draft: '11.2m',
      avgSpeed: '12.3',
      maxSpeed: '14.1',
      latestSpeed: `13 (${future(-1, 16, 30)})`,
      destination: 'FUJAIRAH',
      eta: future(2, 8, 0),
    },
    synMaxInfo: {
      objectId: '512847391',
      imageCapturedTime: '2024-11-02 14:22:18 UTC',
      imageSource: 'Planet Scope',
      status: 'Confirmed',
      latitude: '17.432901',
      longitude: '63.881245',
      heading: '142',
      shipLength: '190',
      shipWidth: '32',
      shipType: 'Bulk Carrier',
      shipSubtype: 'Handymax',
    },
  },
  celestine: {
    id: 'celestine',
    name: 'Celestine',
    flag: '\u{1F1F8}\u{1F1EC}',
    imo: '9321456',
    mmsi: '367890123',
    shipId: 'b7c8d9e0-f1a2-3456-7890-abcdef012345',
    latestEvent: 'Light',
    shipType: 'Container Ship',
    aisInfo: {
      latitude: '10.205634',
      longitude: '67.934521',
      width: '48',
      length: '366',
      shipType: 'Container Ship',
      buildYear: '2019',
      heading: '85',
      draft: '14.5m',
      avgSpeed: '18.2',
      maxSpeed: '22.7',
      latestSpeed: `19 (${future(0, 8, 0)})`,
      destination: 'MUMBAI',
      eta: future(1, 14, 0),
    },
    synMaxInfo: {
      objectId: '629184503',
      imageCapturedTime: '2024-10-29 08:45:33 UTC',
      imageSource: 'Planet Scope',
      status: 'Confirmed',
      latitude: '10.205634',
      longitude: '67.934521',
      heading: '85',
      shipLength: '366',
      shipWidth: '48',
      shipType: 'Container Ship',
      shipSubtype: 'Post-Panamax',
    },
  },
  'meridian-star': {
    id: 'meridian-star',
    name: 'Meridian Star',
    flag: '\u{1F1F5}\u{1F1E6}',
    imo: '9678234',
    mmsi: '412345678',
    shipId: 'c4d5e6f7-8901-2345-6789-0abcdef12345',
    latestEvent: 'Spoofing',
    shipType: 'Cargo',
    aisInfo: {
      latitude: '14.876543',
      longitude: '54.321098',
      width: '28',
      length: '155',
      shipType: 'Cargo',
      buildYear: '2011',
      heading: '310',
      draft: '8.7m',
      avgSpeed: '10.5',
      maxSpeed: '13.8',
      latestSpeed: `11 (${future(0, 7, 10)})`,
      destination: 'SALALAH',
      eta: future(3, 20, 0),
    },
    synMaxInfo: {
      objectId: '847291036',
      imageCapturedTime: '2024-10-25 17:33:12 UTC',
      imageSource: 'Planet Scope',
      status: 'Confirmed',
      latitude: '14.876543',
      longitude: '54.321098',
      heading: '310',
      shipLength: '155',
      shipWidth: '28',
      shipType: 'Cargo',
      shipSubtype: 'General Cargo',
    },
  },
  'unknown': {
    id: 'unknown',
    name: 'Unknown',
    flag: null,
    imo: null,
    mmsi: null,
    shipId: null,
    latestEvent: 'Unattributed',
    shipType: 'Tanker',
    aisInfo: {},
    synMaxInfo: {
      objectId: '483830654',
      imageCapturedTime: '2024-10-18 09:03:46 UTC',
      imageSource: 'Planet Scope',
      status: 'Preview',
      latitude: '24.119738',
      longitude: '58.785984',
      heading: '227',
      shipLength: '113',
      shipWidth: '22',
      shipType: 'Tanker',
      shipSubtype: 'Unassigned',
    },
  },
  'wisdom-star': {
    id: 'wisdom-star',
    name: 'Wisdom Star',
    flag: '\u{1F1F5}\u{1F1E6}',
    imo: '9534871',
    mmsi: '356789012',
    shipId: 'e9f0a1b2-c3d4-5678-9012-3456789abcde',
    latestEvent: 'Light',
    shipType: 'Tanker',
    aisInfo: {
      latitude: '23.456789',
      longitude: '58.123456',
      width: '42',
      length: '274',
      shipType: 'Tanker',
      buildYear: '2017',
      heading: '195',
      draft: '12.3m',
      avgSpeed: '11.8',
      maxSpeed: '15.2',
      latestSpeed: `12 (${future(0, 6, 45)})`,
      destination: 'JEBEL ALI',
      eta: future(2, 12, 0),
    },
    synMaxInfo: {
      objectId: '738291045',
      imageCapturedTime: '2024-11-05 11:12:07 UTC',
      imageSource: 'Planet Scope',
      status: 'Confirmed',
      latitude: '23.456789',
      longitude: '58.123456',
      heading: '195',
      shipLength: '274',
      shipWidth: '42',
      shipType: 'Tanker',
      shipSubtype: 'VLCC',
    },
  },
  'sea-falcon': {
    id: 'sea-falcon',
    name: 'Sea Falcon',
    flag: '\u{1F1EC}\u{1F1F7}',
    imo: '9601233',
    mmsi: '237654321',
    shipId: 'aa11bb22-cc33-dd44-ee55-ff6677889900',
    latestEvent: 'Dark',
    shipType: 'Tanker',
    aisInfo: {
      latitude: '19.884512',
      longitude: '60.442310',
      width: '44',
      length: '250',
      shipType: 'Tanker',
      buildYear: '2013',
      heading: '118',
      draft: '10.9m',
      avgSpeed: '9.4',
      maxSpeed: '12.6',
      latestSpeed: `10 (${future(0, 9, 20)})`,
      destination: 'No info',
      eta: 'No info',
    },
    synMaxInfo: {
      objectId: '910238475',
      imageCapturedTime: '2024-11-08 05:41:22 UTC',
      imageSource: 'Planet Scope',
      status: 'Confirmed',
      latitude: '19.884512',
      longitude: '60.442310',
      heading: '118',
      shipLength: '250',
      shipWidth: '44',
      shipType: 'Tanker',
      shipSubtype: 'Suezmax',
    },
  },
  'orion-dawn': {
    id: 'orion-dawn',
    name: 'Orion Dawn',
    flag: '\u{1F1F5}\u{1F1E6}',
    imo: '9712344',
    mmsi: '351234987',
    shipId: 'bb22cc33-dd44-ee55-ff66-001122334455',
    latestEvent: 'AIS',
    shipType: 'Tanker',
    aisInfo: {
      latitude: '16.223145',
      longitude: '62.109887',
      width: '48',
      length: '333',
      shipType: 'Tanker',
      buildYear: '2020',
      heading: '204',
      draft: '20.1m',
      avgSpeed: '12.0',
      maxSpeed: '15.9',
      latestSpeed: `13 (${future(0, 10, 5)})`,
      destination: 'FUJAIRAH',
      eta: future(2, 18, 0),
    },
    synMaxInfo: {
      objectId: '102938475',
      imageCapturedTime: '2024-11-09 12:15:44 UTC',
      imageSource: 'Planet Scope',
      status: 'Confirmed',
      latitude: '16.223145',
      longitude: '62.109887',
      heading: '204',
      shipLength: '333',
      shipWidth: '48',
      shipType: 'Tanker',
      shipSubtype: 'VLCC',
    },
  },
  'nordic-crown': {
    id: 'nordic-crown',
    name: 'Nordic Crown',
    flag: '\u{1F1F1}\u{1F1F7}',
    imo: '9588122',
    mmsi: '636098123',
    shipId: 'cc33dd44-ee55-ff66-0011-223344556677',
    latestEvent: 'Light',
    shipType: 'Bulk Carrier',
    aisInfo: {
      latitude: '12.556398',
      longitude: '65.778211',
      width: '36',
      length: '229',
      shipType: 'Bulk Carrier',
      buildYear: '2016',
      heading: '77',
      draft: '13.4m',
      avgSpeed: '11.1',
      maxSpeed: '14.3',
      latestSpeed: `12 (${future(0, 8, 40)})`,
      destination: 'MUMBAI',
      eta: future(1, 22, 0),
    },
    synMaxInfo: {
      objectId: '203847561',
      imageCapturedTime: '2024-11-03 09:27:19 UTC',
      imageSource: 'Planet Scope',
      status: 'Confirmed',
      latitude: '12.556398',
      longitude: '65.778211',
      heading: '77',
      shipLength: '229',
      shipWidth: '36',
      shipType: 'Bulk Carrier',
      shipSubtype: 'Panamax',
    },
  },
  'azure-horizon': {
    id: 'azure-horizon',
    name: 'Azure Horizon',
    flag: '\u{1F1F2}\u{1F1F9}',
    imo: '9655401',
    mmsi: '229345671',
    shipId: 'dd44ee55-ff66-0011-2233-445566778899',
    latestEvent: 'Spoofing',
    shipType: 'Container Ship',
    aisInfo: {
      latitude: '9.874521',
      longitude: '68.334190',
      width: '51',
      length: '399',
      shipType: 'Container Ship',
      buildYear: '2021',
      heading: '91',
      draft: '15.8m',
      avgSpeed: '19.7',
      maxSpeed: '24.2',
      latestSpeed: `20 (${future(0, 7, 55)})`,
      destination: 'COLOMBO',
      eta: future(1, 10, 0),
    },
    synMaxInfo: {
      objectId: '304857612',
      imageCapturedTime: '2024-11-07 15:52:38 UTC',
      imageSource: 'Planet Scope',
      status: 'Confirmed',
      latitude: '9.874521',
      longitude: '68.334190',
      heading: '91',
      shipLength: '399',
      shipWidth: '51',
      shipType: 'Container Ship',
      shipSubtype: 'Ultra Large',
    },
  },
}

export const detections = [
  // --- Invictus (9 detections) ---
  { id: 1, shipId: 'invictus', type: 'ais', lng: 63.0, lat: 18.0, date: ago(0, 9, 53) },
  { id: 2, shipId: 'invictus', type: 'light', lng: 60.5, lat: 13.0, date: ago(1, 7, 43) },
  { id: 3, shipId: 'invictus', type: 'dark', lng: 62.0, lat: 8.5, date: ago(2, 14, 21) },
  { id: 4, shipId: 'invictus', type: 'spoofing', lng: 59.5, lat: 17.2, date: ago(2, 10, 5) },
  { id: 5, shipId: 'invictus', type: 'sts', stsPartner: 'wisdom-star', lng: 59.0, lat: 10.0, date: ago(3, 7, 43) },
  { id: 6, shipId: 'invictus', type: 'dark', lng: 57.0, lat: 12.0, date: ago(4, 8, 12) },
  { id: 32, shipId: 'invictus', type: 'dark', lng: 61.8, lat: 16.2, date: ago(0, 10, 40) },
  { id: 33, shipId: 'invictus', type: 'spoofing', lng: 64.2, lat: 19.2, date: ago(0, 12, 10) },
  { id: 43, shipId: 'invictus', type: 'sts', stsPartner: 'wisdom-star', lng: 66.1, lat: 17.0, date: ago(0, 13, 55) },

  // --- Tiffani (9 detections) ---
  { id: 7, shipId: 'tiffani', type: 'light', lng: 60.0, lat: 19.5, date: ago(0, 11, 20) },
  { id: 8, shipId: 'tiffani', type: 'dark', lng: 66.0, lat: 15.0, date: ago(1, 16, 30) },
  { id: 10, shipId: 'tiffani', type: 'spoofing', lng: 67.0, lat: 12.5, date: ago(3, 9, 45) },
  { id: 11, shipId: 'tiffani', type: 'sts', stsPartner: 'celestine', lng: 62.0, lat: 16.0, date: ago(4, 14, 0) },
  { id: 12, shipId: 'tiffani', type: 'light', lng: 69.0, lat: 14.0, date: ago(5, 6, 30) },
  { id: 13, shipId: 'tiffani', type: 'ais', lng: 56.0, lat: 9.0, date: ago(6, 18, 50) },
  { id: 34, shipId: 'tiffani', type: 'dark', lng: 62.7, lat: 14.9, date: ago(0, 13, 5) },
  { id: 35, shipId: 'tiffani', type: 'spoofing', lng: 69.4, lat: 13.4, date: ago(0, 15, 25) },
  { id: 44, shipId: 'tiffani', type: 'sts', stsPartner: 'celestine', lng: 67.2, lat: 15.8, date: ago(0, 16, 5) },

  // --- Celestine (7 detections) ---
  { id: 14, shipId: 'celestine', type: 'ais', lng: 68.0, lat: 10.0, date: ago(0, 8, 0) },
  { id: 15, shipId: 'celestine', type: 'light', lng: 65.0, lat: 7.0, date: ago(1, 12, 15) },
  { id: 16, shipId: 'celestine', type: 'dark', lng: 54.0, lat: 10.0, date: ago(2, 18, 45) },
  { id: 18, shipId: 'celestine', type: 'sts', stsPartner: 'meridian-star', lng: 66.0, lat: 19.0, date: ago(4, 11, 30) },
  { id: 19, shipId: 'celestine', type: 'sts', stsPartner: 'tiffani', lng: 70.0, lat: 9.0, date: ago(5, 15, 45) },
  { id: 36, shipId: 'celestine', type: 'dark', lng: 63.7, lat: 9.7, date: ago(0, 9, 40) },
  { id: 37, shipId: 'celestine', type: 'spoofing', lng: 71.2, lat: 11.3, date: ago(0, 16, 35) },

  // --- Meridian Star (7 detections) ---
  // id 39 (below) is the latest and anchors the briefing pulse; nearby siblings
  // (21, 38) and the STS demo marker 201 were spread out so they don't crowd it.
  { id: 20, shipId: 'meridian-star', type: 'ais', lng: 54.0, lat: 15.0, date: ago(0, 7, 10) },
  { id: 21, shipId: 'meridian-star', type: 'spoofing', lng: 57.8, lat: 11.2, date: ago(1, 19, 25) },
  { id: 23, shipId: 'meridian-star', type: 'sts', stsPartner: 'invictus', lng: 53.0, lat: 7.0, date: ago(3, 20, 40) },
  { id: 24, shipId: 'meridian-star', type: 'sts', stsPartner: 'wisdom-star', lng: 54.2, lat: 10.4, date: ago(4, 5, 55) },
  { id: 25, shipId: 'meridian-star', type: 'dark', lng: 64.0, lat: 20.5, date: ago(5, 10, 10) },
  { id: 38, shipId: 'meridian-star', type: 'dark', lng: 53.6, lat: 18.6, date: ago(0, 11, 55) },
  { id: 39, shipId: 'meridian-star', type: 'spoofing', lng: 56.2, lat: 15.3, date: ago(0, 17, 15) },

  // --- Wisdom Star (7 detections) ---
  // id 41 (below) is the latest and anchors the briefing pulse; nearby siblings
  // (27, 28, 40) were spread out so they don't crowd it.
  { id: 27, shipId: 'wisdom-star', type: 'light', lng: 66.8, lat: 23.2, date: ago(0, 6, 45) },
  { id: 28, shipId: 'wisdom-star', type: 'ais', lng: 58.8, lat: 18.5, date: ago(1, 14, 20) },
  { id: 29, shipId: 'wisdom-star', type: 'dark', lng: 60.0, lat: 20.5, date: ago(2, 9, 30) },
  { id: 30, shipId: 'wisdom-star', type: 'sts', stsPartner: 'meridian-star', lng: 57.2, lat: 12.1, date: ago(4, 5, 55) },
  { id: 31, shipId: 'wisdom-star', type: 'ais', lng: 60.5, lat: 24.5, date: ago(5, 11, 15) },
  { id: 40, shipId: 'wisdom-star', type: 'dark', lng: 65.8, lat: 18.8, date: ago(0, 8, 50) },
  { id: 41, shipId: 'wisdom-star', type: 'spoofing', lng: 62.6, lat: 22.0, date: ago(0, 14, 45) },

  // --- Unknown (2 detections) ---
  { id: 26, shipId: 'unknown', type: 'unattributed', lng: 64.2, lat: 24.3, date: ago(0, 9, 3) },
  { id: 42, shipId: 'unknown', type: 'spoofing', lng: 63.5, lat: 16.2, date: ago(0, 13, 30) },

  // --- N-ship STS marker examples (prototype) ---
  // A demo row just south of the main cluster (~6°N, 56–68°E) showing how the
  // dynamic STS icon reads for 2, 3, 4, 5, and 6 participating ships. `stsShips`
  // is the full participant list (drives how many ships the panel shows) and
  // `stsShipTypes` is each ship's most-recent detection type (drives the marker's
  // colored segments). The two lists are index-aligned.
  { id: 201, shipId: 'invictus', type: 'sts', stsPartner: 'wisdom-star', stsShips: ['invictus', 'wisdom-star'], stsShipTypes: ['light', 'unattributed'], lng: 60.2, lat: 12.0, date: ago(0, 12, 0) },
  { id: 202, shipId: 'invictus', type: 'sts', stsPartner: 'wisdom-star', stsShips: ['invictus', 'wisdom-star', 'meridian-star'], stsShipTypes: ['light', 'ais', 'unattributed'], lng: 66.2, lat: 11.3, date: ago(0, 12, 0) },
  { id: 203, shipId: 'invictus', type: 'sts', stsPartner: 'wisdom-star', stsShips: ['invictus', 'wisdom-star', 'meridian-star', 'celestine'], stsShipTypes: ['light', 'dark', 'ais', 'unattributed'], lng: 60.7, lat: 8.4, date: ago(0, 12, 0) },
  { id: 204, shipId: 'invictus', type: 'sts', stsPartner: 'wisdom-star', stsShips: ['invictus', 'wisdom-star', 'meridian-star', 'celestine', 'tiffani'], stsShipTypes: ['light', 'dark', 'spoofing', 'ais', 'unattributed'], lng: 68.6, lat: 16.4, date: ago(0, 12, 0) },
  // 6-ship event: exceeds the prototype's current 2–5 support. The panel's tabs,
  // roster, and network render all 6 dynamically, but the map marker only paints
  // 5 color segments (badge still counts 6) and the hero image falls back to the
  // 5-ship satellite photo (the 6th pin stacks on the 5th). This is the overflow
  // case we're designing for.
  { id: 205, shipId: 'invictus', type: 'sts', stsPartner: 'wisdom-star', stsShips: ['invictus', 'wisdom-star', 'meridian-star', 'celestine', 'tiffani', 'sea-falcon'], stsShipTypes: ['light', 'dark', 'spoofing', 'ais', 'unattributed', 'dark'], lng: 64.0, lat: 6.4, date: ago(0, 12, 0) },
  // 7- and 10-ship rafts for stress-testing the "Scale to N" (v19) flow. The 10-
  // ship event includes an unattributed vessel, as a real large raft often would.
  { id: 206, shipId: 'invictus', type: 'sts', stsPartner: 'wisdom-star', stsShips: ['invictus', 'wisdom-star', 'meridian-star', 'celestine', 'tiffani', 'sea-falcon', 'orion-dawn'], stsShipTypes: ['light', 'dark', 'spoofing', 'ais', 'unattributed', 'dark', 'ais'], lng: 58.0, lat: 6.0, date: ago(0, 12, 0) },
  { id: 207, shipId: 'invictus', type: 'sts', stsPartner: 'wisdom-star', stsShips: ['invictus', 'wisdom-star', 'meridian-star', 'celestine', 'tiffani', 'sea-falcon', 'orion-dawn', 'nordic-crown', 'azure-horizon', 'unknown'], stsShipTypes: ['light', 'dark', 'spoofing', 'ais', 'unattributed', 'dark', 'ais', 'light', 'spoofing', 'unattributed'], lng: 70.0, lat: 6.0, date: ago(0, 12, 0) },
]
